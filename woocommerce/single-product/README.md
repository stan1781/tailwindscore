...
| Filter | 默认 | 作用 |
|--------|------|------|
| `tailwindscore/pdp/use-section-layout` | `true` | `false` 时回退为单次 `do_action( 'woocommerce_after_single_product_summary' )`，且 **不** 搬迁 WC 核心 callback（见 `inc/woocommerce/hooks/pdp-layout.php`） |
...
- 核心 tabs/upsell/related 在 section 布局下由 `pdp-layout.php` 挂到 `tailwindscore/pdp/section/*`。
